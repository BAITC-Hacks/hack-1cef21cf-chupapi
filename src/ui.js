...
export const badge = task => `<span class="readiness ${task.score >= 90 ? 'priority' : ''}">${icon(task.score >= 90 ? 'spark' : 'check')} ${task.score}<span>/100</span><span class="badge-label">· ${readiness(task.score)}</span></span>`;
...
export function showDialog(html) {
  ...
  dialog.innerHTML = `<button class="icon-button dialog-close" aria-label="Закрыть" data-action="close">${icon('close')}</button>${html}`;
  if (!dialog.open) dialog.showModal();
}
export function empty(title, text, action = 'reset', button = 'Ко всем задачам') {
