...
export const badge = task => currentAssessment(task) ? `<span class="readiness ${task.score >= 90 ? 'priority' : ''}">${icon(task.score >= 90 ? 'spark' : 'check')} ${task.score}<span>/100</span><span class="badge-label">· ${tr(readiness(task.score))}</span></span>` : `<span class="readiness">${tr("Готовность не оценена")}</span>`;
