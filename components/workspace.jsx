...
export function MyChallenges() {
  ...
  if (role === "Student")
    return (
      <>
        <PageHeader title={t("My Challenges")} />
        <div className="empty-state">
          <FileText size={32} />
          <h2>{t("Your business workspace")}</h2>
          <p>{t("Use My Proposals to track your student proposals.")}</p>
          <button
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle={t("From first thought to the next big thing.")}
        action={
      ...
      <div className="workspace-intro">
        <span className="eyebrow">{t("BUSINESS WORKSPACE")}</span>
        <h2>{t("Your ideas, taking shape.")}</h2>
        <p>{t("Your challenges are saved in the shared SQLite database.")}</p>
      </div>
...
export function Applications() {
  ...
  return (
    <>
      <PageHeader
        title={t(role === "Business" ? "Applications" : "My Proposals")}
        subtitle={t(
          role === "Business"
      ...
      <div className="workspace-intro">
        ...
        <p>
          {t(
            "AI never selects a team. Businesses manually accept or reject each proposal.",
          )}
        </p>
      ...
      <div className="application-filter">
        <label>
          ...
          {t(" ")}
          <select
            aria-label={t("Filter proposals by challenge")}
            value={challengeFilter}
            onChange={(e) => setChallengeFilter(e.target.value)}
          >
            <option value="">{t("All challenges")}</option>
            {t(
              db.challenges
