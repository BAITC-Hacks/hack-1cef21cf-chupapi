    ...
    <>
      <section className="rail-card score-panel">
        <div className="row-between">
          <span className="tiny-label">{t("TASK READINESS")}</span>
        ...
        <p className="score-caption">
          {t("Completeness score \xB7 relevance is reviewed separately by AI.")}
        </p>
        {t(
            ...
            Object.entries(score.breakdown).map(([k, value]) => (
              <div key={k}>
                ...
                <div className="mini-track">
                  <span
                    style={{
                      width: (value / scoreWeights[k]) * 100 + "%",
                    }}
                  />
                </div>
              </div>
        ...
        <details className="score-details">
          <summary>{t("Transparent scoring rules")}</summary>
          <p>
            {t(
              "Each field earns 0 for missing information, then 40%, 60%, 80% or 100% for 3\u201319, 20\u201339, 40\u201379 or 80+ characters. Success criteria earn 5 extra points for a numeric target. Contact and collaboration each contribute 5. This measures completeness, not feasibility.",
            )}
          </p>
        </details>
      </section>
      <section className="rail-card doctor">
        ...
        <span className="tiny-label">
          {t(score.total >= 90 ? "FINAL CHECK" : "HOW TO REACH 90+")}
        </span>
        {t(
          suggestions.length ? (
            <ul>
        ...
        <small>
          {t(
            "Guidance only. Missing business facts are never filled in automatically.",
          )}
        </small>
      </section>
    </>
